...
CURSOR C1 IS SELECT 
    ...
    ,NULL BOX_NUMBER
    ,'FTE' AGENCY -- take the last 2 digits
	...
	, decode(TRANSPSTAT_TRANSP_STATUS_CODE,'04','17',TRANSPSTAT_TRANSP_STATUS_CODE) DEVICE_STATUS_INT_ID -- map status '04-closed' to '17-revoked'
FROM PA_ACCT_TRANSP;
...
BEGIN
  ...
  LOOP
    ...
    begin
      select nvl(rtrim(ltrim(TRANSP_SOURCE)),'TRANSCORE'),TRAY_TRAY_NUM||TRAY_CASE_CASE_NUM||TRAY_CASE_LOT_LOT_NUM,
	  TRAY_TRAY_NUM,TRAY_CASE_CASE_NUM
      into DM_DEVICE_INFO_tab(i).MANUFACTURER,
	  DM_DEVICE_INFO_tab(i).BOX_NUMBER,
	  DM_DEVICE_INFO_tab(i).TRAY_NUMBER,DM_DEVICE_INFO_tab(i).CASE_NUMBER
      from PA_INV_TRANSP 
      where INVTRANSP_TRANSP_ID=DM_DEVICE_INFO_tab(i).DEVICE_NUMBER
            and rownum<=1;
	  begin
	    case when DM_DEVICE_INFO_tab(i).MANUFACTURER=1 then DM_DEVICE_INFO_tab(i).MANUFACTURER:='TRANSCORE';
		     when DM_DEVICE_INFO_tab(i).MANUFACTURER=2 then DM_DEVICE_INFO_tab(i).MANUFACTURER:='BESTPASS';
		     when DM_DEVICE_INFO_tab(i).MANUFACTURER=3 then DM_DEVICE_INFO_tab(i).MANUFACTURER:='PREPASS';
		     when DM_DEVICE_INFO_tab(i).MANUFACTURER=4 then DM_DEVICE_INFO_tab(i).MANUFACTURER:='TAGSWAP';
		     when DM_DEVICE_INFO_tab(i).MANUFACTURER=5 then DM_DEVICE_INFO_tab(i).MANUFACTURER:='Nationalpass';
		end case;
	  end;
      exception 
        when others then null;
	...
	/* get PA_TRANSP_STATUS_CODE.TRANSP_STATUS_DESC for DEVICE_STATUS */
    begin
      select TRANSP_STATUS_DESC into DM_DEVICE_INFO_tab(i).DEVICE_STATUS from PA_TRANSP_STATUS_CODE 
      where TRANSP_STATUS_CODE=DM_DEVICE_INFO_tab(i).DEVICE_STATUS
            and rownum<=1;
      exception 
        when others then null;
        DM_DEVICE_INFO_tab(i).DEVICE_STATUS:='UNDEFINED';
    end;
