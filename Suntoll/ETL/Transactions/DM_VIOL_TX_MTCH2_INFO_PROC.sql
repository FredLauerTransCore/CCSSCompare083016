...
-- JOIN WITH JOIN TO TXN_ID OF PA_LANE_TXN TO TXN_ID OF VB_TRANSPONDER_INFO RETURNS TRANSPONDER_ID
...
--    ,NULL PREV_EVENT_TYPE  -- VIOL_TX_STATUS - 1 IF < 0 THEN 0 - *** Xerox please provide translate list
    ,0 VIOL_TX_STATUS
--    ,NULL PREV_VIOL_TX_STATUS  -- VIOL_TX_STATUS - 1 IF < 0 THEN 0
...
-- (IF NOT EXISTS, THEN 'N')
    ,NULL IS_CLOSED -- ST_ACTIVITY_PAID
    ,CREATED_ON DMV_RETURN_DATE -- EVENT_OWNER_ADDR_VEHICLE_ACCT - REFER TO EXTRACT RULE ABOVE
...
--  IF ST_ACTIVITY_PAID.COLL_COURT_FLAG = 'COLL'  AND AMT_CHARGED <> 0 THEN PAID_ON_COLLECTIONS
    ...
    ,NULL DMV_MAKE_ID -- REFER TO EXTRACT RULE ABOVE (#2)
