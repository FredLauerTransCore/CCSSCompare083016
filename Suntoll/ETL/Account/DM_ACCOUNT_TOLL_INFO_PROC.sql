...
--    UFM_LANE_TXN_INFO.HOST_UFM_TOKEN TX_EXTERN_REF_NO -- JOIN TO TXN_ID OF PA_LANE_TXN RETURN HOST TO UFM_TOKEN
    (select HOST_UFM_TOKEN from UFM_LANE_TXN_INFO where TXN_ID = lt.TXN_ID)  TX_EXTERN_REF_NO
    ...
    ,0 LANE_STATE
    ,0 LANE_HEALTH
--    ,pp.AGENCY_ID PLAZA_AGENCY_ID -- Join ST_INTEROP_AGENCIES and PA_PLAZA on ENT_PLAZA_ID to PLAZA_ID
    ,(select AGENCY_ID from ST_INTEROP_AGENCIES where ENT_PLAZA_ID = pp.PLAZA_ID)  PLAZA_AGENCY_ID -- Join ST_INTEROP_AGENCIES and PA_PLAZA on ENT_PLAZA_ID to PLAZA_ID
    ...
    ,(select ACCTTYPE_ACCT_TYPE_CODE from PA_ACCT where ACCT_NUM=kl.ACCT_NUM)
...
--AND lt.EXT_PLAZA_ID = ia.ENT_PLAZA_ID (+)
--AND TRANSPONDER_ID ENDS WITH '2010' 
--AND subtr(lt.TRANSPONDER_ID, ENDS '2010' 
; -- Source
