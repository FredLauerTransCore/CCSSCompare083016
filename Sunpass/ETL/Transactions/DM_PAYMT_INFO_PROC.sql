...
CURSOR C1 IS SELECT 
    ...
    ,NULL CC_TXN_REF_NUM
FROM PA_PURCHASE where PURSTAT_PUR_STATUS_CODE in ('08','10');
...
BEGIN
  ...
  LOOP
    ...
    begin
      select PRODUCT_PUR_PRODUCT_CODE , substr(PRODUCT_PUR_PRODUCT_CODE,1,1) ,
	  substr(PRODUCT_PUR_PRODUCT_CODE,1,1) ,VES_REF_NUM 
	  into 
      ...
      where PUR_PUR_ID =to_number(DM_PAYMT_INFO_tab(i).PAYMENT_REFERENCE_NUM) and rownum<=1;
    ...
    begin
	  ...
	  CC_TXN_REF_NUM ,
	  EMP_EMP_CODE, 
	  decode(length(PUR_CREDIT_EXP_DATE),4,'20','')||substr(PUR_CREDIT_EXP_DATE,decode(length(PUR_CREDIT_EXP_DATE),4,-2,6,-4,0)) ,
	  EMP_EMP_CODE, 
  ...
  EXCEPTION
  WHEN OTHERS THEN
     DBMS_OUTPUT.PUT_LINE('ERROR CODE:xx '||SQLCODE);
     DBMS_OUTPUT.PUT_LINE('ERROR MSG:xx '||SQLERRM);
END;
/
SHOW ERRORS
