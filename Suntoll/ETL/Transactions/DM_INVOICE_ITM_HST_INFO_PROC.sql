...
CREATE OR REPLACE PROCEDURE DM_INVOICE_ITM_HST_INFO_PROC 
...
BEGIN
  ...
  LOOP
    ...
    FOR i IN 1 .. DM_INVOICE_ITM_HST_INFO_tab.COUNT LOOP

      begin
        IF i=1 then
          v_trac_etl_rec.BEGIN_VAL := DM_INVOICE_ITM_HST_INFO_tab(i).ACCOUNT_NUMBER;
        end if;    /*Bulk select */
        
--  DBMS_OUTPUT.PUT_LINE('1) DM_INVOICE_ITM_HST_INFO_tab('||i||').SUB_CATEGORY: '||DM_INVOICE_ITM_HST_INFO_tab(i).SUB_CATEGORY);
        select distinct
...
--JOIN DOCUMENT_ID of ST_DOCUMENT_INFO w DOCUMENT_ID of VB_ACTIVITY where ST_DOCUMENT_INFO.DOC_TYPE_ID in ('1','2')
          WHEN va.COLL_COURT_FLAG is null and va.KS_LEDGER_EVENT_TYPE_ID = '8'
            and DM_INVOICE_ITM_HST_INFO_tab(i).SUB_CATEGORY in ('1','2') -- DOC_TYPE_ID
          THEN 'OTHER'
...
--JOIN DOCUMENT_ID of ST_DOCUMENT_INFO w CHILD_DOC_ID of VB_ACTIVITY where ST_DOCUMENT_INFO.DOC_TYPE_ID ='3'
          WHEN vac.COLL_COURT_FLAG is null and vac.KS_LEDGER_EVENT_TYPE_ID in ('38','42') 
            and DM_INVOICE_ITM_HST_INFO_tab(i).SUB_CATEGORY = '3' -- DOC_TYPE_ID
          THEN 'UTCTOLL'
...
--  and ST_ACCT_FLAGS.FLAG_TYPE ='9' and END_DATE is null, 
          WHEN (SELECT 'Y' 
                ...
                WHERE saf.ACCT_NUM = DM_INVOICE_ITM_HST_INFO_tab(i).ACCOUNT_NUMBER 
...
--          
--          va.KS_LEDGER_EVENT_TYPE_ID = '89'  and DM_INVOICE_ITM_HST_INFO_tab(i).SUB_CATEGORY in ('1','2') 
--          THEN 'REGSTOP'
-------------------------------------------------------------------------------------
...
--          ELSE 'None'      
          ELSE 'REGSTOP2'
         END    
        into  DM_INVOICE_ITM_HST_INFO_tab(i).SUB_CATEGORY
        from  VB_ACTIVITY va
              ,VB_ACTIVITY vac  -- Child
        where va.DOCUMENT_ID = DM_INVOICE_ITM_HST_INFO_tab(i).INVOICE_NUMBER
        and   va.CHILD_DOC_ID = vac.DOCUMENT_ID (+)
        and   rownum<=1  -- Verify what record to get ??
        ;
--  DBMS_OUTPUT.PUT_LINE('2) DM_INVOICE_ITM_HST_INFO_tab('||i||').SUB_CATEGORY: '||DM_INVOICE_ITM_HST_INFO_tab(i).SUB_CATEGORY);
        exception 
          when no_data_found then
          DM_INVOICE_ITM_HST_INFO_tab(i).SUB_CATEGORY := 'FTE Undefined';
          when others then null;
          DM_INVOICE_ITM_HST_INFO_tab(i).SUB_CATEGORY := 'Multi-value';
          DBMS_OUTPUT.PUT_LINE('2) INVOICE_NUMBER: '||DM_INVOICE_ITM_HST_INFO_TAB(i).INVOICE_NUMBER);
...
--  DBMS_OUTPUT.PUT_LINE('3) DM_INVOICE_ITM_HST_INFO_tab('||i||').SUB_CATEGORY: '||DM_INVOICE_ITM_HST_INFO_tab(i).SUB_CATEGORY);
...
--        where kl.ID = va.LEDGER_ID),0) LANE_TX_ID
        ...
        begin
          ...
          into  --DM_INVOICE_ITM_HST_INFO_tab(i).PAYABLE,
                DM_INVOICE_ITM_HST_INFO_tab(i).LANE_TX_ID
          from  ST_ACTIVITY_PAID sap,
                KS_LEDGER kl
          where sap.DOCUMENT_ID = DM_INVOICE_ITM_HST_INFO_tab(i).INVOICE_NUMBER
          and   sap.LEDGER_ID = kl.id
--          group by kl.PA_LANE_TXN_ID
          ;
        exception 
          when others then null;
--          DM_INVOICE_ITM_HST_INFO_tab(i).PAYABLE := 0;
          DM_INVOICE_ITM_HST_INFO_tab(i).LANE_TX_ID := 0;
        end;
...
--          ELSE '0'
         END    
        into  DM_INVOICE_ITM_HST_INFO_tab(i).LEVEL_INFO
        from  VB_ACTIVITY
        where DOCUMENT_ID = DM_INVOICE_ITM_HST_INFO_tab(i).INVOICE_NUMBER
        and rownum=1  -- Verify what record to get ??
        ;
      exception 
        when no_data_found then
-- WHEN COLL_COURT_FLAG is null      and DOCUMENT_ID is null     THEN 'UNBILLED'
          DM_INVOICE_ITM_HST_INFO_tab(i).LEVEL_INFO := 'UNBILLED';
        when others then --null;
          DM_INVOICE_ITM_HST_INFO_tab(i).LEVEL_INFO := '0';
          v_trac_etl_rec.result_code := SQLCODE;
          v_trac_etl_rec.result_msg := SQLERRM;
          v_trac_etl_rec.proc_end_date := SYSDATE;
          update_track_proc(v_trac_etl_rec);
...
--                ,sum(nvl(AMT_CHARGED,0) - nvl(TOTAL_AMT_PAID,0)) 
          ...
          into  DM_INVOICE_ITM_HST_INFO_TAB(i).STATUS
 --               ,DM_INVOICE_ITM_HST_INFO_TAB(i).CREDITS
                ,DM_INVOICE_ITM_HST_INFO_TAB(i).PAID_AMOUNT
          from  
          ((select AMT_CHARGED 
                  ,TOTAL_AMT_PAID
          from  VB_ACTIVITY
        ...
        exception 
          when others then null;
          DBMS_OUTPUT.PUT_LINE('2) INVOICE_NUMBER: '||DM_INVOICE_ITM_HST_INFO_TAB(i).INVOICE_NUMBER);
          DM_INVOICE_ITM_HST_INFO_TAB(i).STATUS := 'CLOSED';
          DM_INVOICE_ITM_HST_INFO_TAB(i).PAID_AMOUNT := 0;
          v_trac_etl_rec.result_code := SQLCODE;
          v_trac_etl_rec.result_msg := SQLERRM;
          v_trac_etl_rec.proc_end_date := SYSDATE;
          update_track_proc(v_trac_etl_rec);
...
--  Use idfference to determine if dismissed or not (IF 0 THEN 'DISPUTED' ELSE 'CLOSED') 
      ...
      if DM_INVOICE_ITM_HST_INFO_tab(i).STATUS != 'OPEN' then
        begin
      ...
      IF DM_INVOICE_ITM_HST_INFO_tab(i).PAID_AMOUNT is null then
        DM_INVOICE_ITM_HST_INFO_tab(i).PAID_AMOUNT := 0;
      end if;
      ...
      begin
        select CREATED_ON
              ,CREATED_ON
        into   DM_INVOICE_ITM_HST_INFO_tab(i).CREATED
              ,DM_INVOICE_ITM_HST_INFO_tab(i).LAST_UPD
        from  PA_ACCT
        where ACCT_NUM = DM_INVOICE_ITM_HST_INFO_tab(i).ACCOUNT_NUMBER
        ;
      exception 
        when others then null;
        DM_INVOICE_ITM_HST_INFO_tab(i).CREATED := SYSDATE;
        DM_INVOICE_ITM_HST_INFO_tab(i).LAST_UPD := SYSDATE;
      end;

      if DM_INVOICE_ITM_HST_INFO_tab(i).LANE_TX_ID is null then
        DM_INVOICE_ITM_HST_INFO_tab(i).LANE_TX_ID := 0;
      End if;

      if DM_INVOICE_ITM_HST_INFO_tab(i).SUB_CATEGORY is null then
        DM_INVOICE_ITM_HST_INFO_tab(i).SUB_CATEGORY := 'REGSTOP2';
      End if;

--  DBMS_OUTPUT.PUT_LINE('ACCT- '||i||' - '||DM_INVOICE_ITM_HST_INFO_tab(i).ACCOUNT_NUMBER);
      v_trac_etl_rec.track_last_val := DM_INVOICE_ITM_HST_INFO_tab(i).ACCOUNT_NUMBER;
...
grant execute on DM_INVOICE_ITM_HST_INFO_PROC to public;
commit;
