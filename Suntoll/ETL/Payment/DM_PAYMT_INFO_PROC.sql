...
CURSOR C1 IS SELECT 
    ...
    ,nvl(pay.PAYTYPE_PAYMENT_TYPE_CODE,'None') PAY_TYPE
...
--    ,NULL REVERSED  -- PA_PURCHASE_DETAIL.PRODUCT_PUR_PRODUCT_CODE 
    ,substr(trim(nvl(pd.PRODUCT_PUR_PRODUCT_CODE,'N')),1,1) REVERSED  -- PA_PURCHASE_DETAIL.PRODUCT_PUR_PRODUCT_CODE 
----    ,(select PROD_ABBRV from PA_PUR_PRODUCT where PUR_PRODUCT_CODE = pd.PRODUCT_PUR_PRODUCT_CODE) REVERSED    ?
    ...
    ,pay.PUR_CREDIT_ACCT_NUM CREDIT_CARD_NUMBER
    ,pay.CHK_ACCT_NUM BANK_ACCOUNT_NUMBER
    ,pay.ABA_NUMBER BANK_ROUTING_NUMBER
    ,pay.CHECK_NUM CHECK_NUMBER
-- DERIVED  PA_PURCHASE_DETAIL.RODUCT_PUR_PRODUCT_CODE ?
    ,substr(trim(pd.PRODUCT_PUR_PRODUCT_CODE),1,1) NSF_FEE -- DERIVED  PA_PURCHASE_DETAIL.RODUCT_PUR_PRODUCT_CODE ?
    ,pay.PUR_PUR_ID PAYMENT_REFERENCE_NUM
    ,pay.EMP_EMP_CODE EMPLOYEE_NUMBER    
