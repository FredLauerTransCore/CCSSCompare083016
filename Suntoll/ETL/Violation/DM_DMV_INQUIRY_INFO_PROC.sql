...
IS SELECT 
    ...
    ,'0' X_HIST_FLG
    ,decode(nvl(ev.ACCT_NUM,0),0,'NULL',ev.ACCT_NUM) X_ACCOUNT_ID  -- JOIN TO EVENT_ADDRESS TABLE TO GAIN ACCOUNT_id
    ,nvl(ea.ADDR1,'NULL-none') X_ADDR_LINE_1
    ,nvl(ea.ADDR2,'NULL-none') X_ADDR_LINE_2
    ,nvl(ea.CITY,'NULL-none') X_CITY
    ,decode(nvl(ea.ID,0),0,'NULL-0',ea.ID) X_CONTACT_ID
    ,decode(nvl(ea.COUNTRY_CODE,0),0,'NULL-',ea.COUNTRY_CODE) X_COUNTRY
